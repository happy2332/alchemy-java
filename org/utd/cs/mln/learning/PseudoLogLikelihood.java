...
public class PseudoLogLikelihood {
    ...
    public PseudoLogLikelihood(State state) {
        ...
        int numGps = state_.groundMLN.groundPredicates.size();
        for (int g = 0; g < numGps; g++) {
            int numVals = state_.groundMLN.groundPredicates.get(g).numPossibleValues;
    ...
    public double getPseudoLogLikelihood() {
        ...
        for(int gpId = 0 ; gpId < numGps ; gpId++) {
            ...
            String predName = state_.groundMLN.groundPredicates.get(gpId).symbol.symbol;
    ...
    private void setWtsPerPredPerVal() {
        int numGroundPreds = satCounts.size();
        MLN mln = state_.mln;
        for (int gpId = 0; gpId < numGroundPreds; gpId++) {
            Map<Integer, List<Integer>> satCountsPerPred = satCounts.get(gpId);
            int numPossibleVals = state_.groundMLN.groundPredicates.get(gpId).numPossibleValues;
