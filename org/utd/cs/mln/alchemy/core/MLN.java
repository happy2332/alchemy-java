...
//	}
	...
	public static Formula create_new_formula(Formula formula)
	{
		...
		List<WClause> newClauses = new ArrayList<WClause>();

		// We must maintain one-to-one correspondence between terms i.e. for every old term,
		// there is a new term, so that when a term repeats in a formula, it's reference
		// must be right.
		List<Term> oldTermsSeen = new ArrayList<Term>();
		List<Term> newTermsSeen = new ArrayList<Term>();

		for(WClause clause : formula.clauses)
		{
			newClauses.add(create_next_new_clause(clause, oldTermsSeen, newTermsSeen));
		}
		Formula newFormula = new Formula(newClauses, formula.weight, false);
		newFormula.terms = newTermsSeen;
		return newFormula;
	}

	// Here termsSeen is the list of terms seen so far while creating the formula
	public static WClause create_next_new_clause(WClause clause, List<Term> oldTermsSeen, List<Term> newTermsSeen) {
		...
		new_clause.valTrue = new ArrayList<Integer>(clause.valTrue);

		new_clause.atoms = new ArrayList<Atom>(clause.atoms.size());
		// In this loop, copy each atom's hypercube, and terms
		for (int i = 0; i < clause.atoms.size(); i++) {
			...
			for(Term term : clause.atoms.get(i).terms)
			{
				// if this term is already seen in oldTerms, then find its corresponding new one
				// and add it to new atom
				int index = oldTermsSeen.indexOf(term);
				if(index != -1)
				{
					new_clause.atoms.get(i).terms.add(newTermsSeen.get(index));
				}
				// else create a new term, add old term to oldtermsseen and new term to newtermsseen
				else
				{
					oldTermsSeen.add(term);
					Term newTerm = new Term(term);
					newTermsSeen.add(newTerm);
					new_clause.atoms.get(i).terms.add(newTerm);
				}
			}
		}
